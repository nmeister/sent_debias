...
# takes in sentence, changes all person names to 'john' or 'mary' and returns the sentence
def change_names(sent):
        ...
        for ent in doc.ents:
            ...
            if ent.label_ == u'PERSON':
                ...
                a = ent.text
                if len(a.split(' ')) > 1:
                    a = a.split(' ')[0]  #genderize only takes first name
                    
                x = d.get_gender(a)
                
                if x == 'male' or x  == 'mostly_male':
                    sent = sent.replace(a, 'John')
                elif x == 'female'or x == 'mostly_female':
